...
public class GenerateAuthenticationFailedSamlResponseTest extends StubVerifyHubAppRuleTestBase {
    ...
    public void generateSamlResponseTest() throws Exception {
        Map<String, String> formData = new HashMap<String, String>() {{
            ...
            put("scenario", "AUTHENTICATION_FAILED");
        }};
    ...
    public void generateSamlResponseFormPageTest() throws Exception {
        ...
        assertThat(html).contains("<h1 class=\"heading-large\">Send a 'AUTHENTICATION_FAILED' Response</h1>");
        assertThat(html).contains("<form action=\"/send-authentication-failed-saml-response\" method=\"POST\">");
    ...
    public void sendSamlResponseTest() throws Exception {
        ...
        Response response = client.target(getUriForPath("/send-authentication-failed-saml-response"))
            .request()
            .header("Content-Type", "application/x-www-form-urlencoded")
            .post(Entity.form(new MultivaluedHashMap<>(formData)));

        Map<String, String> formDataMap = objectMapper.readValue(
            decode(response.getCookies().get("sendAuthenticationFailedResponse").getValue()),
            new TypeReference<Map<String, String>>() {
            }
        );
    ...
    public void sendSamlResponseFormPageTest() throws Exception {
        ...
        Response response = client.target(getUriForPath("/send-authentication-failed-saml-response"))
            .request()
            .cookie(new NewCookie("sendAuthenticationFailedResponse", encode(objectMapper.writeValueAsString(formData))))
            .get();
