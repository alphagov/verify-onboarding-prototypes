...
public class GenerateRequstErrorSamlResponseTest extends StubVerifyHubAppRuleTestBase {
    ...
    public void generateSamlResponseFormPageTest() throws Exception {
        ...
        assertThat(html).contains("<h1 class=\"heading-large\">Send a 'REQUEST_ERROR' Response</h1>");
        assertThat(html).contains("<form action=\"/send-request-error-saml-response\" method=\"POST\">");
    ...
    public void sendSamlResponseTest() throws Exception {
        ...
        Response response = client.target(getUriForPath("/send-request-error-saml-response"))
            .request()
            .header("Content-Type", "application/x-www-form-urlencoded")
            .post(Entity.form(new MultivaluedHashMap<>(formData)));

        Map<String, String> formDataMap = objectMapper.readValue(
            decode(response.getCookies().get("sendRequestErrorResponse").getValue()),
            new TypeReference<Map<String, String>>() {
            }
        );
    ...
    public void sendSamlResponseFormPageTest() throws Exception {
        ...
        Response response = client.target(getUriForPath("/send-request-error-saml-response"))
            .request()
            .cookie(new NewCookie("sendRequestErrorResponse", encode(objectMapper.writeValueAsString(formData))))
            .get();
