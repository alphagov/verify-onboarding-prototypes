...
public class GenerateCancellationSamlResponseTest extends StubVerifyHubAppRuleTestBase {
    ...
    public void generateSamlResponseFormPageTest() throws Exception {
        ...
        assertThat(html).contains("<h1 class=\"heading-large\">Send a 'CANCELLATION' Response</h1>");
        assertThat(html).contains("<form action=\"/send-cancellation-saml-response\" method=\"POST\">");
    ...
    public void sendSamlResponseTest() throws Exception {
        ...
        Response response = client.target(getUriForPath("/send-cancellation-saml-response"))
            .request()
            .header("Content-Type", "application/x-www-form-urlencoded")
            .post(Entity.form(new MultivaluedHashMap<>(formData)));

        Map<String, String> formDataMap = objectMapper.readValue(
            decode(response.getCookies().get("sendCancellationResponse").getValue()),
            new TypeReference<Map<String, String>>() {
            }
        );
    ...
    public void sendSamlResponseFormPageTest() throws Exception {
        ...
        Response response = client.target(getUriForPath("/send-cancellation-saml-response"))
            .request()
            .cookie(new NewCookie("sendCancellationResponse", encode(objectMapper.writeValueAsString(formData))))
            .get();
