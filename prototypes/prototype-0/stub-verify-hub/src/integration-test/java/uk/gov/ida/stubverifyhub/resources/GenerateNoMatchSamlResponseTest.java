...
public class GenerateNoMatchSamlResponseTest extends StubVerifyHubAppRuleTestBase {
    ...
    public void generateSamlResponseFormPageTest() throws Exception {
        ...
        assertThat(html).contains("<h1 class=\"heading-large\">Send a 'NO_MATCH' Response</h1>");
        assertThat(html).contains("<form action=\"/send-no-match-saml-response\" method=\"POST\">");
    ...
    public void sendSamlResponseTest() throws Exception {
        ...
        Response response = client.target(getUriForPath("/send-no-match-saml-response"))
            .request()
            .header("Content-Type", "application/x-www-form-urlencoded")
            .post(Entity.form(new MultivaluedHashMap<>(formData)));

        Map<String, String> formDataMap = objectMapper.readValue(
            decode(response.getCookies().get("sendNoMatchResponse").getValue()),
            new TypeReference<Map<String, String>>() {
            }
        );
    ...
    public void sendSamlResponseFormPageTest() throws Exception {
        ...
        Response response = client.target(getUriForPath("/send-no-match-saml-response"))
            .request()
            .cookie(new NewCookie("sendNoMatchResponse", encode(objectMapper.writeValueAsString(formData))))
            .get();
